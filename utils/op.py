...
def transition(measure, N, **measure_args):
    ...
    if measure == 'lagt':
        ...
        A = np.eye(N) / 2 - np.tril(np.ones((N, N)))    # tril:返回下三角
        B = b * np.ones((N, 1))
    if measure == 'tlagt':
    ...
    if measure == 'glagt':
        ...
        B = ss.binom(alpha + np.arange(N), np.arange(N))[:, None]   # binom:二项式系数

        L = np.exp(.5 * (ss.gammaln(np.arange(N)+alpha+1) - ss.gammaln(np.arange(N)+1)))    # gammaln(x)=ln(|gamma(x)|)
        A = (1./L[:, None]) * A * L[None, :]
    ...
    # Legendre (scaled)
        ...
        A = T @ M @ np.linalg.inv(T)    # np.linalg.inv：矩阵求逆
        B = np.diag(T)[:, None]
